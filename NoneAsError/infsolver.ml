...
(fun f c -> let n = Char.code c in
  ...
  (** val convert_ZF_to_IAZF_Exp :
      ...
      coq_ZExp
      ->
      SIM.InfS.IA.coq_ZExp **)
  
  let rec convert_ZF_to_IAZF_Exp = function
  | ZExp_Var v ->
    SIM.InfS.IA.ZExp_Var
      v
  | ZExp_Const c ->
    (match c with
     | ZE_Fin v ->
       SIM.InfS.IA.ZExp_Const
         (Some
  ...
  | ZBF_Const b ->
    if b
    then SIM.InfS.IA.ZBF_Const
           (Obj.magic
             Three_Val.VTrue)
    else SIM.InfS.IA.ZBF_Const
           (Obj.magic
             Three_Val.VFalse)
  | ZBF_Lt (e1,
            e2) ->
  ...
  (** val convert_FAZF_to_ZF_Exp :
      SIM.InfS.FA.coq_ZExp
      ->
      Coq_sv.var
      coq_ZExp **)
  
  let rec convert_FAZF_to_ZF_Exp = function
  | SIM.InfS.FA.ZExp_Var v ->
    ZExp_Var
      v
  ...
  | SIM.InfS.FA.ZF_BF bf ->
    ZF_BF
      (convert_FAZF_to_ZF_BF
        bf)
  | SIM.InfS.FA.ZF_And (f1,
                        f2) ->
    ZF_And
      ((convert_FAZF_to_ZF
         f1),
  ...
  | SIM.InfS.FA.ZF_Imp (f1,
    ...
    ZF_Or
      ...
      (convert_FAZF_to_ZF
        f2))
  | SIM.InfS.FA.ZF_Not g ->
    ZF_Not
      (convert_FAZF_to_ZF
        g)
  | SIM.InfS.FA.ZF_Forall (v,
                           q,
                           g) ->
    ZF_Forall_Fin
      (v,
